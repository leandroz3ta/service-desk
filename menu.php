...
print "</head><body class='menu alternative-body'>";
	...
	if ($_GET['sis']=="o") {
		$where = "<div class='list-group'><a class='list-group-item active' href='".$ocoDirPath."abertura.php' target='centro'>".TRANS('MNL_OCORRENCIAS')."</a>";
		print "<script>window.parent.frames['centro'].location = '".$ocoHome."'</script>";
		$menu = "treemenu2";
	} else
	if ($_GET['sis']=="i"){
		$where = "<div class='list-group'><a class='list-group-item active' href='".$invDirPath."abertura.php' target='centro'>".TRANS('MNL_INVENTARIO')."</a>";
		print "<script>window.parent.frames['centro'].location = '".$invHome."'</script>";

		$menu="treemenu1";
	} else
	if ($_GET['sis']=="a"){
		$where = "<div class='list-group'><a class='list-group-item active' href='#' target='centro'>".TRANS('MNL_ADM')."</a>";//<img src='".$phplmDirPath."menuicons/".$iconsPath."sysadmin.png'>
		$menu="treemenu3";
		print "<script>window.parent.frames['centro'].location = '".$admHome."'</script>";
	// para usuarios simples
	} else
	if ($_GET['sis']=="s"){
		$where = "<div class='list-group'><a class='list-group-item active' href='".$ocoDirPath."abertura_user.php?action=listall' target='centro'>".TRANS('MNL_OCORRENCIAS')."</a>";
	...
	if ($_GET['sis']=="h"){
		...
		$where = "<div class='list-group'><a class='list-group-item active' href='home.php' target='centro'>".TRANS('MNS_HOME')."</a>";
	...
	// para usuarios simples
		...
		$where = "<div class='list-group'><a class='list-group-item active'href='menu.php' target='centro'>".TRANS('MNS_HOME')."</a>";
	}
