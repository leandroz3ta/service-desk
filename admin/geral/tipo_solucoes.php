<?php 
  ...
  */session_start();
	...
	$conect=$conec->conecta('MYSQL');
        ...
        print "<BR><B>".TRANS('ADM_SCRIPT_SOLUTION','Administração de scripts de soluções').":</B><BR>";
	...
	if ((isset($_GET['action'])  && $_GET['action']=="incluir") && (!isset($_POST['submit']))) {
		...
		print "<B>".TRANS('CADASTRE_SCRIPT_SOLUTION','Cadastro de scripts de solução').":<br>";
...
?>
	...
	<!--
		function valida(){
			var ok = validaForm('idTipo','','Descrição',1);
