...
##################################################
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_operator']) || empty($rowconf)) {
			...
			print "<TD width='30%' align='left' bgcolor=".BODY_COLOR."><input type='text' class='disable' value='".$_SESSION['s_usuario']."' readonly></TD>";
		} else {
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_date']) || empty($rowconf)) {
			print "<TD width='20%' align='left' bgcolor=".TD_COLOR.">".TRANS('OCO_FIELD_DATE_OPEN','Data de abertura').":</TD>";
                	print "<TD width='30%' align='left' bgcolor=".BODY_COLOR."><input type='text' name='data_abertura' class='disable' value='".date("d/m/Y H:i:s")."' readonly></TD>";//datab($hoje)
		}
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_mail']) || empty($rowconf)) {
			print "<td bgcolor='".TD_COLOR."'>".TRANS('OCO_FIELD_SEND_MAIL_TO','Enviar e-mail para').":</td>".
				"<td colspan='3'><input type='checkbox' value='ok' name='mailAR' checked>".TRANS('OCO_FIELD_AREA','')."&nbsp;&nbsp;".
